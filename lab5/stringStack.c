...
// Static global variables to manage the stack
static char *stack[STACK_SIZE];  // Array to hold stack elements (strings)
static int top = 0;              // Index representing the top of the stack

/**
 * push(thing2push) - Adds a string to the top of the stack.
...
void push(char *thing2push) {
    ...
    stack[top++] = thing2push;  // Store string and increment top index
}
...
char *pop() {
    ...
    return stack[--top];  // Decrement top and return the popped element
}

/**
 * isEmpty() - Returns a nonzero value if the stack is empty, otherwise 0.
 */
int isEmpty() {
    return top == 0;
}
